...
def generate_iot_devices(n: int = 8) -> List[Dict]:
    ...
    for i in range(n):
        ...
        for task in random.sample(TASK_POOL[device_type], k=2):
            low, high = TASK_COMPLEXITY_RANGES[device_type] 
        ...
        device = {
            "id": f"Device-{i+1}",
            "type": device_type,
            "cpu": random.randint(500, 3000),              
            "ram": round(random.uniform(0.5, 4), 2),      
            "bandwidth": round(random.uniform(1, 5), 2),     
            "throughput": round(random.uniform(0.1, 1.0), 2),
